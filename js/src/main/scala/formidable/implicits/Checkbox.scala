...
trait Checkbox {
  ...
  class CheckboxBoolRx(default: Boolean)(mods: Modifier *)(implicit ctx: Ctx.Owner) extends FormidableRx[Boolean] {
    val input = scalatags.JsDom.all.input(`type`:="checkbox", mods).render
  ...
  class CheckboxBaseRx[T, Container[_] <: Traversable[_]]
      ...
      (checks: Chk[T] *)
      (implicit ctx: Ctx.Owner) extends FormidableRx[Container[T]] {
