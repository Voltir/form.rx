...
trait Common {
  ...
  class Ignored[T](val default: T) extends FormidableRx[T] {
    override def current: Rx[Try[T]] = Rx { Try(default) }
  ...
  class FormidableBindRx[F <: FormidableRx[Target],Target] extends BindRx[F,Target] {
    ...
    override def unbind(inp: F): rx.Node[Try[Target]] = inp.current
  ...
  class FormidableOptionBindRx[Target, F <: FormidableRx[Target]] extends BindRx[F,Option[Target]] {
    ...
    override def unbind(inp: F): rx.Rx[Try[Option[Target]]] = Rx { Try(inp.current().toOption) }
    override def reset(inp: F): Unit = inp.reset()
  }
